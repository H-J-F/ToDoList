...
public sealed class TaskViewModel(TodoItem item) : ObservableObject
{
    ...
    public TodoItem Item { get => _item; set { _item = value; Raise(); Raise(nameof(IsCompleted)); Raise(nameof(IsVerification)); Raise(nameof(IsDeleted)); Raise(nameof(CheckState)); Raise(nameof(StateHint)); Raise(nameof(CompletionLabel)); } }
    ...
    public string CompletionLabel => IsDeleted ? "已删除 · " + DateTimeOffset.FromUnixTimeMilliseconds(Item.DeletedAt!.Value).ToLocalTime().ToString("MM月dd日 HH:mm") : Item.CompletedAt.HasValue ? "完成于 " + DateTimeOffset.FromUnixTimeMilliseconds(Item.CompletedAt.Value).ToLocalTime().ToString("MM月dd日 HH:mm") : "";
    private bool _busy;

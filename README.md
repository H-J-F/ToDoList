...
仓库：[H-J-F/ToDoList](https://github.com/H-J-F/ToDoList)。[正式版本下载入口](https://github.com/H-J-F/ToDoList/releases)。**当前源码版本为 2.3.1；对应安装包尚未发布到 GitHub Release。**

本地构建提供两版，均为 Windows x64：

| 包名 | 适用情况 |
| --- | --- |
| `Build/ToDoList-2.3.1-win-x64-lite/` | 精简版，需要安装 [.NET 10 Desktop Runtime x64](https://dotnet.microsoft.com/zh-cn/download/dotnet/10.0)，交付文件总量小于 50,000,000 字节 |
| `Build/ToDoList-2.3.1-win-x64-portable/` | 自带运行时，程序压缩为单个 EXE；无需预装 .NET，首次运行会解压必要原生组件 |

Lite 只包含一个 `ToDoList.exe`，缺少运行环境时由原生启动入口提示并提供微软下载入口。

在可写目录中双击 `ToDoList.exe`。全局只运行一个实例，再次启动任一版本会恢复并激活已有窗口；关闭按钮将窗口隐藏到托盘，双击托盘图标恢复，右键“退出 ToDoList”才会结束进程。本地构建只生成程序目录，不自动创建压缩包。两版均在程序同级 `Data` 保存任务和设置，数据互通；体积统计不包含用户数据及单独安装的共享运行时。请保留随程序提供的使用说明和许可证材料。当前构建**尚未进行代码签名**；可信签名及 Microsoft Store 为后续事项。
...
未完成包括黄色待验证，已完成只包括当前完成任务。已删除为可恢复的逻辑删除，使用红色图标、红色正文及红色删除线，不能编辑或勾选；恢复后撤销状态装饰，保留原富文本格式及原完成时间。已删除页签支持右键彻底删除，或开启多选后批量彻底删除；确认后不可恢复。模块支持重命名、删除及长按 500ms 拖动排序；删除模块保留任务。删除待办笔记前自动备份。
...
报告包含书名、筛选条件、导出时间、各状态数量，并按本地添加日期分组；每条任务先显示正文，再紧凑显示彩色状态、模块和添加／完成／删除时间。报告按添加时间从旧到新排序，不受列表已加载数量限制；零结果也可导出概要。报告从同一数据库快照生成，不包含尚未保存的草稿。

MD 为 UTF-8；DOCX 可用 Word 打开；PDF 自动换行分页，使用本机字体生成中文和彩色表情。三种格式统一使用未完成青色、待验证黄色、已完成绿色和已删除红色。导出成功后会打开报告所在目录并选中文件；若目录已经打开则激活现有资源管理器窗口。DOCX 的表情外观随阅读软件而异。导出过程无需安装 Office、连接网络或使用 PDF 打印机，原有整书 `.db` 导出继续用于备份。报告须保存在 Data 目录之外。
